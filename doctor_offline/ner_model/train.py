...
if __name__ == '__main__':
    train(data_loader=data_loader, batch_size=BATCH_SIZE, embedding_dim=EMBEDDING_DIM, hidden_dim=HIDDEN_DIM,
          sentence_length=SENTENCE_LENGTH, num_layers=NUM_LAYERS, epochs=EPOCHS, learning_rate=LEARNING_RATE, tag2id=tag_to_id,
          model_saved_path=model_saved_path, train_log_path=train_log_path, validate_log_path=validate_log_path,
          train_history_image_path=train_history_image_path, data_size=data_size)

    # train(device, data_loader, data_size, batch_size, embedding_dim, hidden_dim,
    #       sentence_length, num_layers, epochs, learning_rate, tag2id,
    #       model_saved_path, train_log_path,
    #       validate_log_path, train_history_image_path)
